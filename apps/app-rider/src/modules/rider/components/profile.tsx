  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <Card style={styles.identity}>
        ...
        <Text style={[styles.name, { color: colors.foreground }]}>{user?.name ?? 'Rider'}</Text>
        <View style={styles.badgeRow}>
          <Badge
            label={rider?.online ? 'Online' : 'Offline'}
            bg={rider?.online ? colors.successMuted : colors.muted}
            fg={rider?.online ? colors.success : colors.mutedForeground}
          />
          {user?.status === 'active' ? (
            <Badge label="Verified" bg={colors.primaryMuted} fg={colors.primary} />
          ) : null}
        </View>
      </Card>
        ...
        <Card style={styles.group}>
          ...
          {rider?.vehicle_type ? (
            <>
              <View style={[styles.sep, { backgroundColor: colors.border }]} />
              <View style={styles.infoRow}>
                <Truck size={16} color={colors.subtleForeground} />
